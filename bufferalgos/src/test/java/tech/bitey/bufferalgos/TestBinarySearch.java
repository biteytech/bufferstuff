...
public class TestBinarySearch {
	...
	private static final int[][][] BINARY_SEARCH_INT = {
		...
		{{0,0}, {}, {IMIN, -1, 0, 1, 2, IMAX}},
		{{0,1}, {0}, {IMIN, -1, 0, 1, 2, IMAX}},
	...
	public void binarySearchInt() {
		...
		for(int[][] config : BINARY_SEARCH_INT) {
			...
			for(int value : config[2]) {
				...
				int actual = BufferAlgos.binarySearch(b, fromIndex, toIndex, value);
				Assertions.assertEquals(expected, actual);
			}
		}
		
		IntBuffer b = IntBuffer.wrap(new int[10]);
		try {			
			BufferAlgos.binarySearch(b, 5, 3, 1);
		...
		try {			
			BufferAlgos.binarySearch(b, -1, 3, 1);
		...
		try {			
			BufferAlgos.binarySearch(b, 5, 11, 1);
			throw new RuntimeException("Expected IndexOutOfBoundsException");
		}
		catch(IndexOutOfBoundsException e) {
			// good
	...
	private static final long[][][] BINARY_SEARCH_LONG = {
		// {fromIndex, toIndex}, {array to search}, {values to search for}
		{{0,0}, {}, {LMIN, -1, 0, 1, 2, LMAX}},
		{{0,1}, {0}, {LMIN, -1, 0, 1, 2, LMAX}},
	...
	public void binarySearchLong() {
		...
		LongBuffer b = LongBuffer.wrap(new long[10]);
		try {			
			BufferAlgos.binarySearch(b, 5, 3, 1);
			throw new RuntimeException("Expected IllegalArgumentException");
		}
	...
	public void binarySearchDouble() {
		...
		DoubleBuffer b = DoubleBuffer.wrap(new double[10]);
		try {			
			BufferAlgos.binarySearch(b, 5, 3, 1);
			throw new RuntimeException("Expected IllegalArgumentException");
		}
